...
with open('data_viewer.txt') as f:
	lines = f.readlines()

shouldTab = False
with open('acmd.txt', 'w') as f:
        for line in lines:
                if '}' in line:
                        f.write('}')
                        shouldTab = False
                ...
                if f_type == "app::lua_bind":
                        ...
                        if shouldTab:
                                f.write("\t"),
                        f.write("{}::{}(acmd->module_accessor{}{});".format(module_name, module_func_name, (', ' if args_str != '' else ''), get_args(args_str)))
                        ...
                        if f_name in WRAPPED_FUNCTIONS:
                                if f_name == 'frame' or f_name == 'wait':
                                        ...
                                        if shouldTab:
                                                f.write("\t"),
                                        f.write("acmd->{}({});".format(f_name, arg))
                                elif f_name == 'ATTACK':
                                        args_str = line[line.index("(")+1:line.index(")")]
                                        
                                        if shouldTab:
                                                f.write("\t"),
                                        f.write("acmd->{}({});".format(f_name, get_args(args_str)))
                        else:
                                module_name = f_name.split("_")[0]
                                module_func_name = '_'.join(f_name.split("_")[2:])
                                args_str = line[line.index("(")+1:line.index(")")]
                                ...
                                if shouldTab:
                                        f.write("\t"),
                                f.write("acmd->wrap({}, {});".format(f_name, get_args(args_str, wrap_args=True)))

with open('acmd.txt', 'r+') as f:
